...
export const fetchCategories = async ({ setGetCategory }: ICatProps) => {
    try{
        const Response = await axios.get("http://10.106.23.101:8888/category/getAllCategories");
        ...
        if( Array.isArray( Response.data)) {
            const fixedData = Response.data.map(item => ({
                ...
                images: item.images.map((img: string) =>
                    img.replace("http://localhost", "http://10.106.23.101")
            )
            }));
...
export const fetchProductsByCatID = async ({
    ...
    try {
      const response: FetchProductsParams = await axios.get(
        `http://10.106.23.101:8888/product/getProductByCatID/${catID}`
      );
      // console.log("API Response:", response.data);
  ...
  export const fetchTrendingProducts = async ({setTrendingProduct }:ITrendingProductProps) => {
    try {
      const response: FetchProductsParams = await axios.get(`http://10.106.23.101:8888/product/getTrendingProduct`);
      console.log("API Response:", response.data);
